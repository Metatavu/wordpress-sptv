...
/**
 * Service location service channel inspector controls
 */
class ServiceLocationServiceChannelInspectorControls extends React.Component<Props, State> {
  ...
  private renderSearchModal = () => {
    return (
      <SearchModal 
        modalTitle={ __("Search Services Locations", 'sptv') }
        inputLabel={ __("Search Services Locations", "sptv") }
        inputHelp={ __("Enter some text to search service locations", "sptv") }
   ...
   * @returns array of service location channels
   */
  private doSearch = async (text: string): Promise<V10VmOpenApiServiceLocationChannel[]> => {
    const lang = "fi";
    const type = "servicelocation";
    const ptv = "v10";
   ...
   */
  private getSearchResultDisplayName = (entity: V10VmOpenApiServiceLocationChannel) => {
   ...
   */
  private onSearchModalSelect = (data: V10VmOpenApiServiceLocationChannel) => {
