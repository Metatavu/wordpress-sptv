...
import { VmOpenApiLocalizedListItem, V10VmOpenApiElectronicChannel } from '../ptv/model/v10/api';
...
/**
 * Electronic service channel inspector controls
 */
class ElectronicServiceChannelInspectorControls extends React.Component<Props, State> {
  ...
  private renderComponentSelect = () => {
    ...
    const options = sptv.electronicServiceChannelBlock.components.map((component) => {
      return {
  ...
  private renderSearchModal = () => {
    return (
      <SearchModal 
        modalTitle={ __("Search Electronic Channel", 'sptv') }
        inputLabel={ __("Search Electronic Channel", "sptv") }
        inputHelp={ __("Enter the first word to search electronic channel", "sptv") }
   ...
   * @returns array of electronic service channels
   */
   ...
   */
  private getSearchResultDisplayName = (entity: V10VmOpenApiElectronicChannel) => {
   ...
   */
  private onSearchModalSelect = (data: V10VmOpenApiElectronicChannel) => {
...
export default ElectronicServiceChannelInspectorControls;